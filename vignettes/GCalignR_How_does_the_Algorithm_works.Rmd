...
Figure 4 illustrates this for one pair-wise comparison of two chromatograms, where the linear evaluated by shifting the Focal sample by -2:+2. Here, the "true" linear shift between reference and sample is approximated by detecting the maximum number of shared peaks in response to the applied shifts. Peaks are counted as shared when their retention times match within a error margin given by 2 x `max_diff_peak2mean` around the reference´s peak. Whenever more than one shift size (including 0!) yield to the same number of shared peaks, the smallest value is taken to avoid overshooting. In this example, shifting the Focal sample by - 1 yields to two shared peaks (indicated by grey rectangles), whereas all other treatments give no peak sharing. However, it is important to note that putatively all three peaks of the Focal sample are shared with the Reference, but are not detected due to non-linear perturbations. We will view on them in the next section.
...
## Sensitivity
Running `align_chromatograms` with default settings will be an good starting point for aligning a dataset. However, every dataset has unique features that will require to change one or more of the key parameters `max_linear_shift`, `max_diff_peak2mean` and `min_diff_peak2peak`.
