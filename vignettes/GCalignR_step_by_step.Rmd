...
* `summary.GCalign`: Summarises the alignment procedure (e.g. documents all arguments to the function call).
...
The attached dataset `seal_peaks` contains data from skin swabs of Antarctic Fur Seals (*Arctocephalus gazella*). 

```{r} 
data("seal_peaks")
length(seal_peaks) # number of individuals 
names(seal_peaks) # names of individuals 
head(seal_peaks[[1]]) # column names and data 
```
...
```{r}
check_input(data = seal_peaks,show_peaks = F) # If show_peaks = T, a histogram of peaks is plotted 
```
...
```{r, eval = FALSE}
seal_peaks <- seal_peaks[1:4] # subset for speed reasons
seal_peaks_aligned <- align_chromatograms(data = seal_peaks, # input data
    ...
    reference = "M48", # name of reference 
...
```
```{r}
data("seal_peaks_aligned") # the package includes the already aligned data set  
``` 
...
gc_heatmap(seal_peaks_aligned,threshold = 0.05) # By default a threshold of 0.05 is used to mark deviations
```


```{r,message=FALSE,fig.width=6,fig.height=6}
plot(seal_peaks_aligned) # Three Diagnostics, can be invoked separetely
```

```{r, eval=FALSE}
summary(seal_peaks_aligned) # Summary of the alingnment procedure
```
...
```{r}
scent <- norm_peaks(seal_peaks_aligned,conc_col_name = "area",rt_col_name = "time",out = "data.frame") # normalise area and return a data frame
scent <- log(scent+1) # commonly used transformation for abundance data to reduce the extent of mean-variance trends
````


### Visualise patterns by ordination plots using the *vegan* 

The `vegan`package offers a variety of useful function for the analysis of multivariate abundance data such as the scent profiles handled here. Check out *?vegan* for a first overview. 

```{r}
data("seal_factors") # factors for individual scent samples
scent <- scent[match(row.names(seal_factors),row.names(scent)),] # bring both to the same order
...
scent_nmds <- cbind(scent_nmds,colony=seal_factors$colony) # add the colony as a factor 
ggplot2::ggplot(data = scent_nmds,ggplot2::aes(MDS1,MDS2,color=colony)) +
    ggplot2::geom_point(size=4) + ggplot2::stat_ellipse(size=2) + ggplot2::labs(title ="", x = "MDS1", y = "MDS2") + ggplot2::theme_bw()
````
## Literature
