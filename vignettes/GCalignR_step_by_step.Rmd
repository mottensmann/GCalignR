...
GCalignR provides a simple means of aligning peaks based of gas chromatography data for chemometric analysis. 
We specifically developed and tested GCalignR as a preprocessing tool prior to the statistical 
analysis of chemicals from animal skin and preen glands. Chromatograms of multiple samples are not directly
...
what might it be due? No idea, but we also didn´t use them obviously).
...
```{r, eval = FALSE} 
install.packages("devtools") devtools::install_github("mastoffel/inbreedR",
...
to align these extracted peaks across individuals using just their retention times (and not the
...
or the height of a peak. `GCalignR` uses these peak data to align the peaks among individuals.
