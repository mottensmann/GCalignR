...
Run the alignment process with default parameters using peaks from `ind1` as a 
reference.

```{r, message=FALSE} 
alignment <- align_chromatograms(data=gc_peak_data,conc_col_name="area", rt_col_name = "RT", reference = "ind1")
```
...
gc_heatmap(alignment) # By default a threshold of 0.05 is used to mark deviations
gc_heatmap(alignment,threshold = 0.03,substance_subset =1:10,samples_subset = 1:10) # Inspect the first ten samples and substances 
```
